...
class GeneralModel extends ChangeNotifier {
  ...
  ScrollController _scrollProController = ScrollController();
  ...
  ScrollController get scrollProController => _scrollProController;
  ...
  Future<void> init() async {
    ...
    insertMessage(
        "Hola Ari, yo soy Yoini tu asistente personal ¿En qué te puedo ayudar?",
        true,
        false);
    _playAudio("init");
    ...
    Future.delayed(const Duration(milliseconds: 1800), () {
      insertMessage(
          "Estas son las funciones que tengo disponibles: ${funciones}.",
          true,
          false);
      _playAudio("init");
  ...
  void scrollToProBottom() {
    if (scrollProController.hasClients) {
      scrollProController.animateTo(
        scrollProController.position.maxScrollExtent,
        duration: const Duration(milliseconds: 300),
        curve: Curves.easeOut,
      );
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (assistantResponse.contains("4")) {
          ...
          insertMessage('Qué tipo de profesional buscas', true, false);

          // navigate to
        }
        if (assistantResponse.contains("5")) {
          // setIntendedInteraction(4, context);

          insertMessage('estos son nuestros mejores cuidadores:', true, true);
  ...
  void insertMessage(String message, bool isBot, bool isList) {
